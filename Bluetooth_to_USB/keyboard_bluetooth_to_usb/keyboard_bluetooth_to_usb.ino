...
void setup() {
  Btd.btdName = "Arduino";  
  bthid[0] = new BTHID(&Btd, PAIR, "0000"); // Create the instances
  bthid[1] = new BTHID(&Btd); // Create the instances
  ///bthid[1].btdName = "Keyboard";
  ...
  bthid[0]->setProtocolMode(HID_RPT_PROTOCOL); // Report Protocol Mode
  ...
  if (Usb.Init() == -1)
    error_blink(400);
  delay( 200 );
  bthid[0]->SetReportParser(KEYBOARD_PARSER_ID, &kbd);
